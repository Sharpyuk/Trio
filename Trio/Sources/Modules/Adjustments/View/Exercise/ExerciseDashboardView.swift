...
struct ExerciseDashboardView: View {
    ...
    var body: some View {
        if let session = coordinator.activeSession {
            ...
            Section {
                Button("Schedule Exercise") { presentedSheet = .schedule }
                ...
                Text(
                    "Correction strength reduces the total positive correction requirement before SMB and temp basal are calculated."
                )
            }
        }
...
private struct ExerciseReportView: View {
    ...
    var body: some View {
        List {
            ...
            LabeledContent(
                "BG start / end",
                value: "\(report.bgStart.map(String.init) ?? "—") / \(report.bgEnd.map(String.init) ?? "—")"
            )
            LabeledContent(
                "BG min / max",
                value: "\(report.bgMinimum.map(String.init) ?? "—") / \(report.bgMaximum.map(String.init) ?? "—")"
            )
            LabeledContent(
                "IOB start / end",
                value: "\(report.iobStart?.formatted() ?? "—") / \(report.iobEnd?.formatted() ?? "—") U"
            )
            ...
            LabeledContent(
                "Audio",
                value: report.announcementsEnabled ? "Every \(report.announcementIntervalMinutes) min" : "Off"
            )
...
private struct ExerciseStatusSection: View {
    ...
    private var timeValue: String {
        ...
        case .preExercise,
             .scheduled: session.scheduledExerciseStart
...
private struct ExerciseHistoryRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack { Text(session.preset.name)
                Spacer()
                Text(session.phase(at: Date()).rawValue.capitalized) }
